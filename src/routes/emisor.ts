...
import { emisorDelTenant, puedeGestionarEmisor } from "./_guards.js";
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!puedeGestionarEmisor(request, parsed.data.cedula)) {
        return reply
          .status(403)
          .send({ error: `Tu credencial no tiene permiso para gestionar el emisor "${parsed.data.cedula}"` });
      }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!puedeGestionarEmisor(request, cedula)) {
        return reply
          .status(403)
          .send({ error: `Tu credencial no tiene permiso para gestionar el emisor "${cedula}"` });
      }
